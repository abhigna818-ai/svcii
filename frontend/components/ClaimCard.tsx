...
function consistencyIcon(c: SatelliteConsistency | null): { symbol: string; color: string; label: string } {
  if (c === 'consistent')  return { symbol: '✓', color: 'var(--green-primary)', label: 'Satellite-consistent' };
  if (c === 'divergent')   return { symbol: '✗', color: 'var(--red)', label: 'Satellite-divergent' };
  if (c === 'inconclusive')return { symbol: '?', color: 'var(--orange)', label: 'Inconclusive' };
  return { symbol: '·', color: 'var(--text-muted)', label: 'Not assessed' };
}
...
function metricColor(t: string | null): string {
  if (t === 'absolute') return 'var(--green-primary)';
  if (t === 'intensity') return 'var(--orange)';
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: '0.875rem', lineHeight: 1.6, color: 'var(--text-primary)', marginBottom: '0.625rem',
          fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem 1rem', alignItems: 'center' }}>
          ...
          {claim.magnitude_pct != null && (
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem',
              color: claim.magnitude_pct < 0 ? 'var(--green-primary)' : 'var(--red)', fontWeight: 500 }}>
