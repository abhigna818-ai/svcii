...
function classificationMeta(c: string | null): { bg: string; border: string; text: string; blurb: string } {
  if (c === 'CONSISTENT')
    return { bg: 'rgba(56,102,65,0.08)', border: 'var(--signal-grn)', text: 'var(--signal-grn)',
  ...
  if (c === 'MAJOR DIVERGENCE')
    return { bg: 'rgba(193,18,31,0.07)', border: 'var(--signal-red)', text: 'var(--signal-red)',
  ...
  if (c === 'WARRANTS INVESTIGATION')
    return { bg: 'rgba(231,111,0,0.08)', border: 'var(--signal-amb)', text: 'var(--signal-amb)',
  ...
  return { bg: 'rgba(231,111,0,0.06)', border: 'var(--signal-amb)', text: 'var(--signal-amb)',
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '2rem',
        alignItems: 'start', paddingBottom: '2rem', borderBottom: '1px solid var(--paper-3)' }}>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '3rem',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px',
            ...
            <div style={{ background: 'var(--paper-2)', padding: '1.25rem' }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', fontWeight: 700,
                color: 'var(--ink)', lineHeight: 1, marginBottom: '0.25rem' }}>
          ...
          {score.e_score && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem',
                ...
                <ScoreBar label="Trend Direction Agreement" value={score.e_score.components.trend_direction} max={40} color="var(--accent)" />
                <ScoreBar label="Magnitude Proportionality" value={score.e_score.components.magnitude} max={30} color="var(--accent)" />
                <ScoreBar label="Temporal Consistency" value={score.e_score.components.temporal} max={20} color="var(--accent)" />
                <ScoreBar label="Disclosure Quality" value={score.e_score.components.disclosure} max={10} color="var(--accent-2)" />
              </div>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)' }}>
          ...
          {score.s_score?.s_score != null && score.s_score.components && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem',
                marginBottom: '0.75rem' }}>
                <ScoreBar label="Land Integrity" value={score.s_score.components.land_integrity} max={100} color="var(--accent-2)" />
                <ScoreBar label="Community Prosperity" value={score.s_score.components.community_prosperity} max={100} color="var(--accent-2)" />
                <ScoreBar label="Supply Chain Land Use" value={score.s_score.components.supply_chain} max={100} color="var(--accent-2)" />
              </div>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5625rem', color: 'var(--muted)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem',
          ...
          {facilities.length > 0 && (
            <div>
              ...
              <div style={{ overflowX: 'auto', border: '1px solid var(--paper-3)',
                borderRadius: '2px', overflow: 'hidden' }}>
                  ...
                  <tbody>
                    {facilities.map(f => (
                      <tr key={f.id}>
                        ...
                        <td style={{ fontFamily: 'var(--font-mono)', textAlign: 'right',
                          color: f.xch4_trend == null ? 'var(--muted)'
                            : f.xch4_trend < -1 ? 'var(--signal-grn)'
                            : f.xch4_trend > 1 ? 'var(--signal-red)' : 'var(--signal-amb)' }}>
                          {f.xch4_trend != null
