...
function CodeBlock({ children }: { children: string }) {
  return (
    <pre style={{ background: 'var(--bg-surface)', border: '1px solid var(--beige-warm)', borderRadius: '2px',
      padding: '1.25rem', overflowX: 'auto', fontFamily: 'var(--font-mono)', fontSize: '0.8125rem',
      lineHeight: 1.7, color: 'var(--text-dark)', marginTop: '1rem', marginBottom: '1rem' }}>
      <code>{children}</code>
    </pre>
  );
}
...
export default function MethodologyPage() {
  return (
    <div className="container-narrow" style={{ paddingTop: '2.5rem', paddingBottom: '4rem' }}>
      ...
      <p style={{ color: 'var(--text-muted-light)', marginBottom: '3rem', lineHeight: 1.7 }}>
      ...
      <Section title="E Score — Environmental (0–100)">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px',
          background: 'var(--beige-warm)', marginBottom: '1.5rem', border: '1px solid var(--beige-warm)',
          ...
          {[
            ...
            <div key={name} style={{ background: 'var(--bg-surface)', padding: '1rem 1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline',
                marginBottom: '0.375rem' }}>
                <span style={{ fontWeight: 500, fontSize: '0.875rem' }}>{name}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem',
                  color: 'var(--green-deep)', fontWeight: 500 }}>{pts}</span>
              </div>
              <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted-light)', lineHeight: 1.6 }}>{desc}</p>
            </div>
          ))}
        </div>
...
if metric_type == 'intensity': raw_score -= 10  # additional penalty`}</CodeBlock>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px',
          ...
          {[
            ...
            <div key={name} style={{ background: 'var(--bg-surface)', padding: '1rem 1.25rem',
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--text-muted-light)',
      ...
      <Section title="Classification">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px',
          background: 'var(--beige-warm)', border: '1px solid var(--beige-warm)', borderRadius: '2px',
          overflow: 'hidden' }}>
          {[
            ['≥ 80', 'CONSISTENT',              'var(--green-deep)', 'Satellite data directionally consistent with ESG claims across all components.'],
            ['60–79','INCONCLUSIVE',             'var(--classification-warrants)', 'Mixed signals. Directional agreement but notable magnitude or temporal discrepancies.'],
            ['40–59','WARRANTS INVESTIGATION',   'var(--classification-warrants)', 'Notable divergence between claimed and observed trends. Warrants further scrutiny.'],
            ['< 40', 'MAJOR DIVERGENCE',         'var(--classification-divergence)', 'Satellite data significantly contradicts stated ESG claims.'],
            ...
            <div key={label} style={{ background: 'var(--bg-surface)', padding: '1rem 1.25rem',
              ...
              <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted-light)', lineHeight: 1.6, margin: 0 }}>{desc}</p>
            </div>
          ))}
        </div>
      </Section>
          ...
          <div key={s.name} style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '1rem', marginBottom: '0.25rem' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem',
                color: 'var(--text-muted-light)' }}>{s.vintage}</span>
            </div>
            <p style={{ fontSize: '0.875rem', color: 'var(--text-muted-light)', lineHeight: 1.6 }}>{s.desc}</p>
          </div>
        ))}
      </Section>
      ...
      <Section title="Limitations">
        <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingLeft: '1.25rem' }}>
          {[
            ...
            <li key={i} style={{ fontSize: '0.875rem', lineHeight: 1.7, color: 'var(--text-muted-light)' }}>{l}</li>
          ))}
        </ul>
      </Section>
      ...
      <div style={{ borderTop: '1px solid var(--beige-warm)', paddingTop: '2rem' }}>
